...
class imscc11_converter extends base_converter
{
    ...
    protected function execute() {
        ...
        if (!$validator->validate($manifest)) {
            ...
            $normalisedresources = $this->normalise_resource_metadata($manifest);
            $normalisedltiresources = $this->normalise_unsupported_lti_resources($manifest);
            if ($normalisedresources || $normalisedltiresources) {
                if ($normalisedresources) {
                    $this->log('normalised resource metadata', backup::LOG_DEBUG, null, 1);
                }
                if ($normalisedltiresources) {
                    $this->log('normalised unsupported LTI resources', backup::LOG_DEBUG, null, 1);
                }
                error_messages::instance()->reset();
                if (!$validator->validate($manifest)) {
    ...
    /**
     * Removes unsupported LTI 1.3 resources from Canvas IMS CC 1.1 exports.
     *
     * @param string $manifest the manifest file path
     * @return int number of resource or outline item nodes removed
     */
    protected function normalise_unsupported_lti_resources($manifest) {
        ...
        $xpath = cc112moodle::newx_path($xmldoc, cc112moodle::$namespaces);
        $resources = $xpath->query('/imscc:manifest/imscc:resources/imscc:resource[@type="imsbasiclti_xmlv1p3"]');
        $changed = 0;

        foreach ($resources as $resource) {
            $identifier = $resource->getAttribute('identifier');
            if ($identifier !== '') {
                $items = $xpath->query('/imscc:manifest/imscc:organizations/imscc:organization//imscc:item[@identifierref]');
                foreach ($items as $item) {
                    if ($item->getAttribute('identifierref') !== $identifier) {
                        continue;
                    }
